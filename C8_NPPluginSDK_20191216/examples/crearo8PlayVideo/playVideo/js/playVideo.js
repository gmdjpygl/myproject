...
var WebClient = {
	...
	*/
	...
	// 本地存储目录
	HasLocalSaveDir: function (dirName) {
		try {
			var dirName = dirName || 'C://WebSdkData';
			
			var operator = NPPILY.Folder.CreateDirectory(dirName);
			if (operator.rv != 0) {
				return false;
			}
	...
	LocalSnapshot: function () {
		try {
			...
			var node = NPPILY.WindowContainers.get("windowbox");
			if (node && node.window) {
				var dirName = 'C://WebSdkData/Snapshot';
				if (WebClient.HasLocalSaveDir(dirName)) {
					NPPILY.Snapshot(node.window, dirName);
				}
			}
		}
		catch (e) {
		...
		try {
			...
			if (node && node.window) {
				if (node.window.status.playvideoing) {
					...
					var dirName = 'C://WebSdkData/Record';
					if (WebClient.HasLocalSaveDir(dirName)) {
						NPPILY.LocalRecord(node.window, dirName);
					}
					...
					if (node.window.status.recording) {
						...
						customMenus.push({key: "localrecord", text: "停止录像"});
					} else {
						$("#opt_record").val("开启录像");
						$("#opt_record").removeClass("buttonSelect");
						customMenus.push({key: "localrecord", text: "开启录像"});
					}
					NPPILY.WindowAttachEvent.UpdateMenuCommand(node.window, customMenus);
				}
			}
