...
var NPPILY = 
{
    ...
    */
    ...
    Plug : 
    {
        ...
        Error : 
        {
            domid : "NPP_nppWarmTipBox", active : false, pluginFile : "MediaPlugin8.exe", html : "",
    ...
    */
    Init : function (initParam)
    {
        ...
        {	
			...
			NPPILY.Plug.Error.pluginFile = initParam.warmTip.pluginFile || "MediaPlugin8.exe";
    ...
    **/
    ...
    params: 
		...
		- options(object) 点播相关参数
		{
			...
			sgIndex(uint) 前端点播时存储器资源索引，缺省为0
			diskLetter: 前端点播时存储器磁盘盘符，比如'A' | 'B' ...
			ivIndex(uint) 点播对象（摄像头）资源索引，缺省为0
			fileFullPath(string) 点播的录像文件路径，如D:/StoragePath/123.avi、/disk/storage/0/123.avi等
	...
	remark:
		...
		- 点播前端CEFS录像，type=NPPILY.Enum.StorageFileType.CEFS时，可选下列参数传入puid、sgIndex、ivIndex、diskLetter、speed、direction、beginTime、endTime
		- 点播前端FAT32（EXT4等）录像，type=NPPILY.Enum.StorageFileType.FAT32时，可选下列参数传入puid、sgIndex、fileFullPath、speed、direction、relativeStartTime
	... 
	*/
		...
		try
		{
			...
			switch (o.type)
			{
				...
				case NPPILY.Enum.StorageFileType.FAT32 : 
					...
					if (!o.fileFullPath)
					{
						NPPUtils.Log(fn, "fileFullPath error~");
						return new NPPILY.Struct.ReturnValueStruct(NrcapError.NRCAP_ERROR);
					}

					// 获取设备Storager资源句柄
					var sgh_operator = NPPILY.NPPSDKCommon.GetHandle
					(
						connectId, 
						o.puid, 
						NPPILY.Enum.PuResourceType.Storager, 
					...
					if (sgh_operator.rv != NrcapError.NRCAP_SUCCESS)
					{
						NPPUtils.Log(fn, "get pu handle error~");
						if (sgh_operator.rv == NrcapError.NRCAP_ERROR)
						{
							sgh_operator.rv = NrcapError.NRCAP_ERROR_HANDLE_ERROR;	
						}
						return new NPPILY.Struct.ReturnValueStruct(sgh_operator.rv);
					}

					var operator = NPPIF.SG_FAT32VODFilePlay
					(
						_connStruct.nc,
						sgh_operator.response,
						o.fileFullPath,
						o.speed,
						o.direction,
						o.relativeStartTime,
    ...
    **/
	/**
    * --------------------------------------------------------------------------------------------------------
    *	- s - remark: Download Control - 2013.11.25 -> 2019.11.21
    * ........................................................................................................
    **/ 
	Download :
	{
				...
				(
					function (item)
					{
						if (item.key == connectId)
						{
							...
							_Sdk_ResetHandle(item.value.dlFAT32FileStore);
		...
		*/
		...
		/*
		---
		fn: StartFAT32FileDownload
		desc: 开始下载FAT32前端存储文件
		author:
			- huzw
		params:
			- connectId(string) * 连接ID
			...
			- sgIndex(uint) * 前端存储器资源索引，默认为0
			- ivIndex(uint) * 抓拍所属视频资源索引，默认为0
			- remoteFilePath(string) * 远程存储全路径，path + name，比如/storage/Record/vod.mp4 | ...
			- localSaveAsFile(string) * 下载保存全路径，如C:/vod.mp4 | ...，上层调用时确保正确的存储路径
		...
		*/
		StartFAT32FileDownload : function (connectId, puid, sgIndex, ivIndex, remoteFilePath, localSaveAsFile)
		{
			return NPPILY.Download.__Sdk_DLControl
			(
			 	"dlfat32file",
				{
					...
					sgIndex: sgIndex || 0,
					ivIndex: ivIndex || 0,
					remoteFilePath: remoteFilePath || null,
					localSaveAsFile: localSaveAsFile || null
				}
			); 
		},
		/*
		---
		fn: StopFAT32FileDownload
		desc: 停止下载FAT32前端存储文件
		author:
			- huzw
		params:
			- connectId(string) * 连接ID
		...
		...
		*/
		StopFAT32FileDownload : function (connectId, dlStreamHandle)
		{
			return NPPILY.Download.__Sdk_DLControl
			(
			 	"stopdl",
		...
		__Sdk_DLControl : function (action, options)
		{
			try
			{
				...
				if (!_dlhash.get(options.connectId))
				{
					...
					(
					 	options.connectId, 
						{
							...
							dlFAT32FileStore: new NPPUtils.Hash()
						}
					);	
				}
				...
				switch (action)
				{
					...
					case "dlfat32file": // - 开始下载FAT32格式存储文件
						if (!options.puid || !NPPILY.puidRex.test(options.puid))
						{
							NPPUtils.Log(fn, "options.puid error~");
							return new NPPILY.Struct.ReturnValueStruct(NrcapError.NRCAP_ERROR_PUID_ERROR);	
						...
						if (!options.remoteFilePath || !options.localSaveAsFile)
						{
							NPPUtils.Log(fn, "options.localSaveAsFile error~");
							return new NPPILY.Struct.ReturnValueStruct(NrcapError.NRCAP_ERROR);	
						}
						...
						var _dlkey = options.puid + 
										"_" + options.sgIndex +
										"_" + options.ivIndex + 
										"_" + options.remoteFilePath +
										"_" + options.localSaveAsFile;

						if (_dlhash.get(options.connectId))
						{
							var _dlstore = _dlhash.get(options.connectId).dlFAT32FileStore || new NPPUtils.Hash();
							if (_dlstore.containsKey(_dlkey))
							{
								// - 判断是否存在同一下载通道
								if (typeof _dlstore.get(_dlkey).dlStreamHandle != 'undefined'
									&& _dlstore.get(_dlkey).dlStreamHandle != null)
								{
									NPPUtils.Log(fn, "fat32 file dl existed, dlkey->" + _dlkey);
									return new NPPILY.Struct.ReturnValueStruct(NrcapError.NRCAP_ERROR_DOWNLOAD_EXISTED, _dlstore.get(_dlkey).dlStreamHandle)
								}
							}
	...
	/**
    * --------------------------------------------------------------------------------------------------------
    *	- e - remark: Download Control - 2013.11.25 -> 2019.11.21
    * ........................................................................................................
    **/
	/**
    * --------------------------------------------------------------------------------------------------------
	...
	*/
	...
	SG_FAT32QueryFiles : function (connectId, puid, sgIndex, ivIndex, intBeginTime, intEndTime, streamType, intOffset, intCount)
	{
		try {
			var fn = 'NPPILY.SG_FAT32QueryFiles';

			// streamType = STORAGE 查录像，= PICTURE 查图片
			var operator = NPPILY.NPPSDKCommon.GetNCResponse
			(
				connectId, 
				"SG_FAT32QueryFiles", 
				puid, 
			...
			if (operator && operator instanceof NPPILY.Struct.ReturnValueStruct)
			{
				if (operator.rv == NrcapError.NRCAP_SUCCESS)
				{
					...
					for (var i = 0; i < operator.response.length; i++)
					{
						...
						if (typeof item.reasons != 'undefined' && item.reasons)
						{
							item.reasons = NPPUtils.UTF8toUnicode(item.reasons);
							try {
								if (typeof item.size != 'undefined')
								{
									item.size = Number(item.size || 0);
								}
								if (typeof item.begin_time != 'undefined')
								{
									item.begin_time = Number(item.begin_time || 0);
								}
								if (typeof item.end_time != 'undefined')
								{
									item.end_time = Number(item.end_time || 0);
								}
								if (typeof item.time != 'undefined')
								{
									item.time = Number(item.time || 0);
								}
								if (typeof item.no != 'undefined')
								{
									item.no = Number(item.no || 0);
								}
								item.reasons = item.reasons.replace(/^,{1}/,'');
							}
							catch (e) {
							}
						}
			...
			{
				operator = new NPPILY.Struct.ReturnValue(NrcapError.NRCAP_ERROR);
			}
        ...
        params:
            ...
            => {
				...
				warmTip(object):
				{
					...
					pluginFile(string) 插件绝对地址或网络地址，默认"MediaPlugin8.exe"
        ...
        */
        InitParamStruct : function (debug, cb, extraParams) 
        {
			...
			this.warmTip = {
				...
				pluginFile : _wt.pluginFile || NPPILY.Plug.Error.pluginFile || "MediaPlugin8.exe",
				html : _wt.html || ""
			};
        },
        // - 调试信息结构
