var _be = BuildEmap = {
	...
	Load: function () {
		if (_be.doResizer == null) {
			_be.doResizer = new NPPUtils.Hash();
		}
	...
	Frame: {
		Init: function () {
			try {
				...
				html.push('<div data-options="region:\'south\',title:\'GPS信息\',height:100,collapsed:true">');
					html.push('<table id="gps-grid-box" class="easyui-treegrid"></table>');
				html.push('</div>');
			...
			try {
				...
				if ($("#gps-grid-box")[0]) {
					...
					if (!inited) {
						...
						var columns = [{
								field: "gps_identity", title: "", width: 0, hidden: true 
							},{
								field: "gps_name", title: "名称", width: 260			   
							}, {
						...
						$("#gps-grid-box").treegrid({
							fit: true,
							border: false,
							collapsible: false,
							singleSelect: true,
							nowrap: false,
							idField: "gps_identity",
							treeField: "gps_name",
							lines: false,
							rownumbers: true,
							fitColumns: false,
		...
		CreateGPSViewBox: function (index, rowData) {
			try {
				...
				var puid = rowData.gps_identity.split("_")[0];
				if (puid && (puNode = _be.Resource.GetPUNode(puid))) {
					var html = [],
						containerID = "gps_ddv-" + index,
						gridkey = rowData.gps_identity;
	...
	Control: {
		...
		CreateBottom: function () {
			try {
				...
				// html.push('<span style="display:inline-block;margin-top:5px;margin-left:5px;"><select id="control-switchmap" class="easyui-combobox"><option value="baidumap" title="">使用百度地图</option><option value="googlemap" title="">使用Google地图</option></select></span>');
	...
	Login: {
		...
		Success: function (operator) {
			try {
				...
				var _connStruct = NPPILY.Connections.get(_be.connectId);
				if (_connStruct) {
					var toHeight = 150, gps_grid_title;
					if (_connStruct.connType != NPPILY.Enum.ConnectionType.Device) {
						toHeight = 290;
						gps_grid_title = "登录服务器 - " +　(_connStruct.systemName || "NVS");
					}
					else {
						gps_grid_title = "直连设备";
					}
					...
					$("#gps-grid-box")
						.treegrid({
							"title": gps_grid_title, 
							iconCls: "icon-tip",
							onClickRow: function (row) {
								_be.Emap.ClickGPSList(row);
							},
							onDblClickRow: function (row) {
								_be.Emap.DblClickGPSList(row);
							}
						});
		...
		DisConnection: function () {
			try {
				...
				if ($("#videos-list-window")[0]) {
					$("#videos-list-window").window("close");
				}
	...
	Resource: {
		...
		Init: function () {
			try {
				...
				var connectId = _be.connectId,
					...
					resource = _be.Resource.PaginationForkResource(connectId);	
				
				_be.Resource.LoadGPSData(resource);
				
				// 级联子域平台资源
				_be.Resource.CascadeResource();
				// - 关闭掩码
				_be.Mask.Hide();
		...
		// 级联资源平台资源
		CascadeResource: function () {
			try {
				var fn = "_be.Resource.CascadeResource";
				
				var connectId = _be.connectId,
					cs = NPPILY.Connections.get(connectId);
				
				if (cs.domainRoad) {
					// 递归获取子域
					var get_cascades = function (dr_parent) {
						try {
							var data = [],
								dr_parent = dr_parent || "";
							$.each(cs.domainRoad, function (index, dr) {
								if (!dr_parent && dr.indexOf(".") != -1) {
									return true;
								}
								else if (dr_parent && dr.indexOf(".") == -1) {
									return true;
								}
								else if (dr.indexOf(".") != -1 && dr.substr(dr.indexOf(".") + 1) != dr_parent) {
									return true;
								}
								data.push({
									gps_identity: dr,
									gps_name: (dr_parent ? dr.substr(0, dr.indexOf(".")) : dr),
									iconCls: "root",
									type: "dr",
									dr: dr,
									dr_parent: dr_parent,
									state: "open",
									children: get_cascades(dr)
								});
							});
							return data;
						}
					...
					var data = get_cascades();
					$("#gps-grid-box").treegrid("append", {data: data});
					
					$.each(cs.domainRoad, function (index, dr) {
						var resource = _be.Resource.PaginationForkResource(connectId, dr);
						_be.Resource.LoadGPSData(resource, dr);
					});
				}
			}
			catch (e) {
		...
		PaginationForkResource: function (connectId, domainRoad) {
			try {
				var fn = "_be.Resource.PaginationForkResource";
				...
				if (GLOBALS.debug) {
					logger(
						fn, 
						$.stringFormatter(
							"dr of {0}, resource count of {1}, data -> {2} )",
							domainRoad,
							resource.length,
							$.toJSON(resource)
						)
		...
		// 嵌入GPS数据
		LoadGPSData: function (resource, drName) {
			try {
				var fn = "_be.Resource.LoadGPSData";
				
				var drName = drName || "",
					connectId = _be.connectId,
					cs = NPPILY.Connections.get(connectId),
					data = [];
					...
					if (cs.connType == NPPILY.Enum.ConnectionType.Device) {
						for (var i = 0; i < item.childResource.length; i++) {
							...
							if (child.type == NPPILY.Enum.PuResourceType.GPS) {
								...
								// - 创建GPS数据接收通道
								var rv = _be.Emap.CreateGPS(item, child);
								if (rv && rv.length > 0) {
									data = data.concat(rv);
								}								
							} 
					...
					if (item.online == 1 && item.enable == 1) {
						switch (item.modelType) {
							...
							case NPPILY.Enum.PuModelType.WENC:
								// - 获取子资源创建GPS数据接收通道
								var rv = _be.Resource.ForkChildResource(item, drName);
								if (rv && rv.length > 0) {
									data = data.concat(rv);
								}
								break;
				...
				if (data && data.length > 0) {
					if (!drName) {
						...
						var dr_node = $("#gps-grid-box").treegrid("find", drName),
							dr_childs = $("#gps-grid-box").treegrid("getChildren", drName);
						if (dr_childs.length <= 0) {
							$("#gps-grid-box").treegrid("append", {
								parent: dr_node.gps_identity,
								data: data
							});
						}
						else {
							var dr_child = dr_childs[0];
							$.each(data, function (index, item) {
								$("#gps-grid-box").treegrid("insert", {
									before: dr_child.gps_identity,
									data: item
								});
							});
						}
		...
		// - 获取子资源
		ForkChildResource: function (puNode, drName) {
			try {
					...
					if (operator.response) {
						...
						for (var i = 0; i < operator.response.length; i++) {
							...
							if (child.type == NPPILY.Enum.PuResourceType.GPS) {
								...
								// - 创建GPS数据接收通道
								var rv = _be.Emap.CreateGPS(puNode, child, drName);
								if (rv && rv.length > 0) {
									data = data.concat(rv);
								}
							} 
		...
		PUOnline: function (puid) {
			try {
				...
				if (puNode && puNode.childResource) {
					...
					for (var i = 0; i < puNode.childResource.length; i++) {
						...
						if (child.type == NPPILY.Enum.PuResourceType.GPS) {
							...
							if (!exists) {
								...
								// - 创建GPS数据接收通道
								var rv = _be.Emap.CreateGPS(puNode, child);		
								if (rv && rv.length > 0) {
									data = data.concat(rv);
								}
							}
					...
					if (data.length > 0) {
						var roots = $("#gps-grid-box").treegrid("getRoots");
						if (roots.length <= 0) {
							$("#gps-grid-box").treegrid("append", {
								data: data
							});
						...
						for (var i = 0; i < puNode.childResource.length; i++) {
							...
							switch (child.type) {
								...
								case NPPILY.Enum.PuResourceType.VideoIn:
									// 停止恢复视频
									_be.Video.SBRServer.NoConflict(puid, child.idx);
									break;
							}
						} 
						// - 停止相关视频
	...
	Emap: {
		...
		CreateGPS: function (puNode, gpsNode, drName) {
			try {
				...
				if (gpsNode instanceof NPPILY.Struct.PUResourceNodeStruct) {
					...
					drName =  drName || "";
					var identity = (drName?(drName + "-"):"") +  puNode.puid + "_" + gpsNode.idx;
					...
					// - 加入到列表
					return [{
						gps_identity: identity,
						gps_name: gpsObj.puName + "(" + gpsObj.gpsNode.name + ")",
						iconCls: "icon-tip",
						type: gpsObj.drName ? "dr_pu" : "pu",
						dr: gpsObj.drName
					}];
				}
			}
			...
			try {
				...
				if (puNode && gpsNode && gpsNode.type == NPPILY.Enum.PuResourceType.GPS) {
					...
					(
						...
						puNode.puid, 
						gpsNode.idx, {
							_HDNDLE: gpsNode._HANDLE
						}
					);
					// - 从列表中移除
					var identity = puNode.puid + "_" + gpsNode.idx;
					$("#gps-grid-box").treegrid("remove", identity);
					
					// - 从数据存储中删除
					var gpsObj = _be.Emap.GPSObjectStore.get(identity);
					if (gpsObj) {
						// - 卸载接收状态通知
						...
						_be.Emap.GPSObjectStore.unset(identity);
					}
				}
			}
			catch (e) {
		...
		GPSData_Callback: function (_ncnStruct) {
			try {
				...
				var identity = _be.Emap.MappingHandleKey[_ncnStruct._HANDLE];
		...
		// - 单击列表项
		ClickGPSList: function (rowData) {
			try {
				var fn = "_be.Emap.ClickGPSList";
				
				if (GLOBALS.debug) logger(fn, "rowData -> " + $.toJSON(rowData));
				
				if (rowData) {
					var key = rowData.gps_identity,
		...
		// 双击列表项
		DblClickGPSList: function (rowData) {
			try {
				var fn = "_be.Emap.DblClickGPSList";

				if (GLOBALS.debug) logger(fn, "rowData -> " + $.toJSON(rowData));
				
				if (rowData) {
					var identity = rowData.gps_identity,
						gpsObj = _be.Emap.GPSObjectStore.get(identity),
						win_dimen_w = $(window).width(),
						win_dimen_h = $(window).height(),
						width = 315,
						height = 175,
						left = (typeof event != "undefined" ? (event.clientX + 10) : win_dimen_w),
						top = (typeof event != "undefined" ? (event.clientY + 10) : win_dimen_h),
						title = $.stringFormatter("视频列表\{{0}\}", gpsObj.puName);
					
					if ($("#videos-list-window")[0]) {
						var options = $("#videos-list-window").window("options");
						width = options.width;
						height = options.height;
					}
					
					if (win_dimen_w < left + width) {
						left = left - width - 10;
					}
					if (win_dimen_h < top + height) {
						top = top - height - 10;
					}
					
					if (!$("#videos-list-window")[0]) {
						$("#gps-grid-box")
							.append('<div id="videos-list-window"></div>')
							.find("#videos-list-window")
							.window({
								title: title,
								left: left,
								top: top,
								...
								minimizable: false,
								maximizable: false,
								content: '<div id="videos-list-window-content" style="width:100%;height:100%;"></div>',
								onResize: function (w, h) {
									if (w && h) {
										$("#videos-list-window-content DIV:last")
											.css({
												height: (h - 85)
											});
									}
							...
							.attr("curPuid", gpsObj.puid);
						
						// 页面改变大小
						_be.doResizer.set("doing_videos-list-window", function (w, h) {
							logger(fn, w + "," + h);
							
							var options = $("#videos-list-window").window("options");
							if (options.left > w || options.top > h) {
								$("#videos-list-window").window("resize", {
									left: w - options.width - 10,
									top: h - options.height - 10
								});								
							}
						});
					}
						...
						if ($("#videos-list-window").attr("curPuid") == gpsObj.puid) {
							return false;
						}
						$("#videos-list-window").attr("curPuid", gpsObj.puid);
					}
					
					if ((puNode = _be.Resource.GetPUNode(gpsObj.puid))) {
						var html = [];
						
						html.push('<div style="width:95%;height:30px;line-height:30px;vertical-align:middle;margin:0px auto;">');
							html.push('<ul style="list-style:none;width:100%;">');
								html.push('<li style="float:left;cursor:pointer;margin-left:10px;border:1px gray dashed;background-color:#BFBFF7;color:#000000;border-radius:4px;padding:0px 5px 0px 5px;" onmouseover="this.style.textDecoration=\'underline\';" onmouseout="this.style.textDecoration=\'none\';" onclick="_be.Video.PopPlayAllVideo(\''+gpsObj.puid+'\');" title="将会弹出一个新的页面进行播放全部视频，理论上只能够播放前十六路视频，超过不进行播放">全部弹出播放</li>');
								html.push('<li style="float:left;cursor:pointer;margin-left:10px;border:1px gray dashed;background-color:#F7BFBF;color:#000000;border-radius:4px;padding:0px 5px 0px 5px;" onmouseover="this.style.textDecoration=\'underline\';" onmouseout="this.style.textDecoration=\'none\';" onclick="_be.Video.PlainPlayAllVideo(\''+gpsObj.puid+'\');" title="在此页面内播放全部视频（所有已经正在播放的视频将会关闭）">全部播放</li>');
							html.push('</ul>');
						html.push('</div>');
						html.push('<div style="width:95%;height:0px;margin:0px auto 2px auto;border-top:1px gray solid;"></div>');
						html.push('<div style="width:95%;height:65px;margin:0px auto;overflow:auto;">');
							html.push('<ul style="list-style:none;">');
							for (var i = 0; i < puNode.childResource.length; i++) {
								var child = puNode.childResource[i];
								if (child.type == NPPILY.Enum.PuResourceType.VideoIn) {
									var coreName = (child.name || child.description);
									html.push('<li style="float:left;cursor:pointer;border:1px gray dashed;background-color:#E4EAEE;#BFF7BF;color:#000000;border-radius:4px;padding:0px 5px 0px 5px;margin:2px 2px 2px 2px;" onmouseover="this.style.textDecoration=\'underline\';" onmouseout="this.style.textDecoration=\'none\';" onclick="_be.Video.PlayVideo(\''+gpsObj.puid+'\', \''+child.idx+'\');" title="'+coreName+'">' + coreName + '</li>'); 	
								}
							}
							html.push('</ul>');
						html.push('</div>');
						
						$("#videos-list-window-content")
							.css("overflow", "hidden")
							.html(html.join(""))
							.find("DIV:last")
							.css({
								height: ($("#videos-list-window").window("options").height - 85)
							});
					}
				}
			}
				...
				if (gpsObj && lastData) {
					...
					if (mapObj && mapObj.markers) {
						...
						htmlstr += "<div>";
							...
							if(gpsObj.polyline == null){
								htmlstr += "<span onmouseover=\"this.style.textDecoration='underline';\" onmouseout=\"this.style.textDecoration='none';\" style=\"cursor:pointer;color:#0464BB;text-decoration:none;\" id=\""+id_pfx+"_polyline\" href=\"javascript:void(0);\" onclick=\"_be.Emap.Polyline('"+gpsObj.puid+"', '"+gpsObj.gps_idx+"','"+gpsObj.drName+"');\">轨迹跟踪</span>&nbsp;";					
							}
							else {
								htmlstr += "<span onmouseover=\"this.style.textDecoration='underline';\" onmouseout=\"this.style.textDecoration='none';\" style=\"cursor:pointer;color:#0464BB;text-decoration:none;\" id=\""+id_pfx+"_polyline\" href=\"javascript:void(0);\" onclick=\"_be.Emap.RemovePolyline('"+gpsObj.puid+"', '"+gpsObj.gps_idx+"','"+gpsObj.drName+"');\">停止跟踪</span>&nbsp;";					
							}  
							if(gpsObj.lock === false){
								htmlstr += "<span onmouseover=\"this.style.textDecoration='underline';\" onmouseout=\"this.style.textDecoration='none';\" style=\"cursor:pointer;color:#0464BB;text-decoration:none;\" id=\""+id_pfx+"_lock\" href=\"javascript:void(0);\" onclick=\"_be.Emap.LockVihicle('"+gpsObj.puid+"', '"+gpsObj.gps_idx+"','"+gpsObj.drName+"');\">锁定车辆</span>&nbsp;";					
							}
							else {
								htmlstr += "<span onmouseover=\"this.style.textDecoration='underline';\" onmouseout=\"this.style.textDecoration='none';\" style=\"cursor:pointer;color:#0464BB;text-decoration:none;\" id=\""+id_pfx+"_lock\" href=\"javascript:void(0);\" onclick=\"_be.Emap.UnLockVihicle('"+gpsObj.puid+"', '"+gpsObj.gps_idx+"','"+gpsObj.drName+"');\">解除锁定</span>&nbsp;";					
							} 
							// - 清除轨迹 ---
							htmlstr += "<span onmouseover=\"this.style.textDecoration='underline';\" onmouseout=\"this.style.textDecoration='none';\" style=\"cursor:pointer;color:#0464BB;text-decoration:none;\" id=\""+id_pfx+"_clear_polyline\" href=\"javascript:void(0);\" onclick=\"_be.Emap.ClearPolylineStore('"+gpsObj.puid+"', '"+gpsObj.gps_idx+"','"+gpsObj.drName+"');\" />清除轨迹</span>";
		...
		// - 跟踪轨迹
		Polyline: function (puid, gpsIndex, drName) {
            try{
				var fn = "_be.Emap.Polyline";
				
				var key = (drName?(drName+"-"):"") + puid + "_" + gpsIndex,
					gpsObj = _be.Emap.GPSObjectStore.get(key);
				if (gpsObj) {
					if(gpsObj.polyline != null) {
						gpsObj.polyline.setPath([]);	
		...
		// - 移除轨迹
		RemovePolyline: function (puid, gpsIndex, drName) {
            try{
		...
		// - 锁定车辆
		LockVihicle: function (puid, gpsIndex, drName) {
            try{
		...
		// - 移除锁定
		UnLockVihicle: function (puid, gpsIndex, drName) {
            try{
		...
		// - 清除轨迹
		ClearPolylineStore: function(puid, gpsIndex, drName) {
			try {
			...
			try{
				...
				if(gpsObj && gpsObj.puid) {
					var puid = gpsObj.puid,
						gi = gpsObj.gps_idx,
						drName = gpsObj.drName,
						key = (drName?(drName+"-"):"") + puid + "_" + gi;
	...
	Video: {
		...
		Clear: function (mode) {
			try {
				...
				NPPILY.WindowContainers.each(function (item) {
					if (item.value.type == NPPILY.Enum.WindowType.VIDEO) {
						_be.Video.ResponseOperation("StopVideo", item.key);	
						if (mode === true) { 
		...
		PlainPlayAllVideo: function (puid) {
			try {
				...
				if (puid && (puNode = _be.Resource.GetPUNode(puid))) {
					// 判断是否在线/使能
					var operator = NPPILY.GetResourceUsable(_be.connectId, puid);
					if (operator.rv == NrcapError.NRCAP_SUCCESS) {
						if (operator.response == 0) {
							alert("设备不在线！");
							return false;
						}
					}
					/*var operator = NPPILY.GetResourceEnable(_be.connectId, puid);
					if (operator.rv == NrcapError.NRCAP_SUCCESS) {
						if (operator.response == 0) {
							alert("设备未使能！");
							return false;
			...
			try {
				...
				if (puid && typeof ivIndex != "undefined") {
					...
					if (puNode) {
						...
						// 规避重复的断开恢复播放信息
						_be.Video.SBRServer.NoConflict(puid, childRes.idx, streamType);
							...
							if (node.active == true) {
								...
								if (node.window) {
									...
									if (node.window.status.playvideoing) {
										// 规避重复的断开恢复播放信息
										_be.Video.SBRServer.UnReg(item.key, true);
								...
								if (play_operator.rv == NrcapError.NRCAP_SUCCESS) {
									...
									$.messager.show({
										title: "提示 - 播放失败",
										msg: "<ul><li>视频名称<font color='blue'>"+node.description.name+"</font></li><li>错误代码<font color='blue'>"+play_operator.rv+"</font></li><li>描述<font color='blue'>"+NrcapError.Detail(play_operator.rv)+"</font></li></ul>",
										timeout: 3000,
										height: 130
									});
								}
					...
					if (item.key == winkey) {
						...
						if (node.window && node.window.status.playvideoing) {
							...
							if (_ncnStruct.status == -1) {
								// 记录视频相关信息
								_be.Video.SBRServer.Reg(item.key);
								_be.Video.ResponseOperation("StopVideo", item.key);
								_objT2.html("").css("width", "1%");
								var tip = (node.description.name || "") + "&nbsp;" + _ncnStruct.statusDesc + "，正在尝试恢复播放";
								_objT1.html(tip).attr("title", tip.replace(/(\&nbsp;)/g, "")).css("width", "98%");
								return false;
							}
		...
		// 流断开重连
		// SBRServer(StreamBrokenRestoreServer)
		SBRServer: {
			interval: 1000,
			timeout: 30000, // 断开重连超时时间（单位毫秒）
			store: new NPPUtils.Hash(),
			Reg: function (winkey) {
				try {
					var fn = "_be.Video.SBRServer.Reg";
					
					NPPILY.WindowContainers.each(function (item) {
						var node = item.value;
						if (item.key == winkey && node.window && node.window.status.playvideoing) {
							_be.Video.SBRServer.store.set(
								item.key, {
									active: true,
									key: item.key,
									puid: node.window.params.puid,
					...
					if (!NPPUtils.Timer.ContainsKey("SBRServer", "Checking")) {
						NPPUtils.Timer.Start();
						NPPUtils.Timer.Set(
							"SBRServer", {
								name: "Checking",
								fu: function () {
									_be.Video.SBRServer.Checking();
									
									var activate = _be.Video.SBRServer.store.any(function (item) {
										return item.value.active == true;
									});
									if (!activate) {
										NPPUtils.Timer.UnSet("SBRServer", "Checking");
									} 
								},
								interval: _be.Video.SBRServer.interval
							}
						);
					}
				}
			...
			UnReg: function (winkey, mode) {
				try {
					var fn = "_be.Video.SBRServer.UnReg";
					
					_be.Video.SBRServer.store.each(function (item) {
						if (item.key == winkey) {
							var winNoder = NPPILY.WindowContainers.get(item.key);
							if (winNoder && winNoder.container) {
								var _objT1 = $(winNoder.container).find(".wintitle .title1");
								_objT1.html("无视频").attr("title", "");
							}
							item.value.active = false;
							if (mode === true) {
								_be.Video.SBRServer.store.unset(item.key);
							}
						}
					});
				}
			...
			Clear: function () {
				_be.Video.SBRServer.store.clear();
			},
			NoConflict: function (puid, ivIdx, streamType) {
				try {
					var fn = "_be.Video.SBRServer.NoConflict";
					
					_be.Video.SBRServer.store.each(function (item) {
						var node = item.value;
						if (node.puid == puid && node.idx == ivIdx && (typeof streamType == "undefined" || node.streamType == streamType)) {
							node.active = false;
			...
			Checking: function () {
				try {
					var fn = "_be.Video.SBRServer.Checking";
					...
					_be.Video.SBRServer.store.each(function (item) {
						...
						if (node.active == false) {
							_be.Video.SBRServer.UnReg(item.key);
							return false;
						}
						else {
							if (cur_time - node.reg_time > _be.Video.SBRServer.timeout) {
								// 超时了，停止恢复视频
								node.active = false;
								if (winNoder && winNoder.container) {
									var _objT1 = $(winNoder.container).find(".wintitle .title1");
								...
								$.messager.show({
									title: "提示 - 超时错误",
									msg: "<ul><li>视频名称<font color='blue'>"+node.name+"</font></li><li>描述<font color='blue'>视频流断开自动重连失败</font></li><li>请重新手动尝试打开</li></ul>",
									timeout: 3000,
									height: 130
								});
								return false;
								...
								if (winNoder && winNoder.window) {
									...
									if (operator.rv == 0) {
										...
										NPPILY.ResizeWindowDimension(winNoder.window, "100%", "100%");
										
										var _HANDLE = winNoder.window.params.ivStreamHandle;
										_be.Video.MappingHandleWinkey[_HANDLE] = item.key;
										_be.NCEvent.HandleCaller.Register(_HANDLE, _be.Video.ResponseStreamHandleCallback);
										
										// - 允许DDraw模式
										if (GLOBALS.defaultDDraw != winNoder.window.status.isddrawing) {
											_be.Video.SwitchDDraw(item.key);
										}
									}
								}
	...
	Resize: function () {
		try {
			...
			var win_dimen_w = $(window).width(),
				win_dimen_h = $(window).height();
			
			_be.doResizer.each(function (item) {
				var resizer = item.value;
				if (typeof resizer == "function") {
					resizer(win_dimen_w, win_dimen_h);
				}			
			});
		}
		catch (e) {
